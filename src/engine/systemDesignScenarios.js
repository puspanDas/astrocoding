...
export const scenarios = [
  ...
  },
  {
    ...
    icon: '🎟️',
    difficulty: 4,
    description: 'Design a highly concurrent ticket booking system like Ticketmaster or BookMyShow. The system must prevent double-booking of seats while serving millions of users during popular event drops.',
    hints: [
      '💡 You need a Load Balancer to distribute the massive traffic spike.',
      '💡 Redis is critical for holding seat reservations for 5-10 minutes (caching available seats).',
      '💡 PostgreSQL guarantees ACID transactions so two people do not buy the exact same seat.',
      '💡 Use a Message Queue (RabbitMQ) to process payment confirmations asynchronously.',
    ],
    requiredComponents: ['web-client', 'load-balancer', 'api-gateway', 'server', 'postgresql', 'redis', 'rabbitmq'],
    requiredConnections: [
      ['web-client', 'load-balancer'],
      ['load-balancer', 'api-gateway'],
      ['api-gateway', 'server'],
    ...
    techStack: 'Languages: Java, Go, or Node.js. DB: PostgreSQL for transactions. Cache: Redis for seat locks.',
    complexity: 'Time: O(1) for seat lookups. Space: O(E * S) for Events and Seats.',
    failureMessage: 'Missing essential components for high concurrency! Ensure you have an API Gateway, Server, Redis (for seat locking), PostgreSQL (for safe transactions), and RabbitMQ.',
    scoring: {
      basePoints: 500,
      bonusComponents: { 'cdn': 25, 'monitoring': 30, 'dns': 15, 'mobile-client': 20 },
      bonusLabel: 'Add CDN, Monitoring, or Mobile Client for extra points!',
    },
    successMessage: '🎉 Perfect! Redis for distributed seat locks and PostgreSQL for ACID transactions is the gold standard for high-concurrency booking engines.',
  },
  {
    id: 'url-shortener',
    ...
    icon: '🔗',
    difficulty: 3,
    description: 'Design a URL shortener service like Bitly. The system must generate short hashes for long URLs and handle billions of fast redirects every month.',
    hints: [
      '💡 This is an extremely read-heavy system (100 reads per 1 write).',
      '💡 An API Gateway should route the traffic.',
      '💡 A fast NoSQL database (MongoDB) is perfect for storing simple key-value pairs of short-to-long URLs.',
      '💡 Redis is mandatory to cache the most popular redirects and achieve sub-10ms latency.',
    ],
    requiredComponents: ['web-client', 'api-gateway', 'server', 'mongodb', 'redis'],
    requiredConnections: [
    ...
    techStack: 'Languages: Go or Node.js. Database: MongoDB or Cassandra. Cache: Redis.',
    complexity: 'Time: O(1) for cache hits. Space: O(N) where N is number of shortened URLs.',
    failureMessage: 'Your URL shortener is missing caching or fast storage. Use an API Gateway, Server, MongoDB (for URL storage), and Redis (for hot URLs).',
    scoring: {
      basePoints: 350,
      bonusComponents: { 'load-balancer': 30, 'dns': 15, 'monitoring': 20 },
      bonusLabel: 'Add Load Balancer or Monitoring to improve resilience!',
    },
    successMessage: '🎉 Excellent! A read-heavy system relies on Redis caching. A NoSQL database like MongoDB is highly scalable for simple short-to-long URL mappings.',
  },
  {
    ...
    icon: '🔔',
    difficulty: 4,
    description: 'Design a scalable system that sends millions of push, SMS, and email notifications. Services must not crash if a third-party email provider goes down.',
    hints: [
      '💡 Do not send notifications synchronously. Decouple them!',
      '💡 A Server should publish notification events to a Message Queue (RabbitMQ).',
      '💡 Serverless functions are great for picking up messages from the queue and actually calling third-party APIs to send them.',
      '💡 Log the delivery statuses in PostgreSQL or MongoDB.',
    ],
    requiredComponents: ['server', 'rabbitmq', 'serverless', 'postgresql'],
    requiredConnections: [
      ['server', 'rabbitmq'],
      ['rabbitmq', 'serverless'],
      ['serverless', 'postgresql'],
    ],
    techStack: 'Queue: RabbitMQ/Kafka. Workers: Serverless Functions. Tracking: PostgreSQL.',
    complexity: 'Time: O(1) queue insertion. Async processing prevents blocking.',
    failureMessage: 'Your notification system is tightly coupled. You must use a Message Queue (RabbitMQ) to decouple your Server from the Serverless workers sending the alerts.',
    scoring: {
      basePoints: 400,
      bonusComponents: { 'mongodb': 30, 'logging': 25, 'monitoring': 30, 'kafka': 40 },
      bonusLabel: 'Add Logging or switch to Kafka for high-throughput event processing!',
    },
    successMessage: '🎉 Great architecture! Decoupling components with RabbitMQ and processing with Serverless workers is how companies ensure no notifications are lost during outages.',
  },
